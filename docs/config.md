...
- **`screenshot_privacy_mode`.** With `skip-monitor`, OpenChronicle enumerates on-screen windows immediately before each screenshot and applies the app, bundle, and window-title denylists to every window, including floating panels. Detection uses local top-level owner/app name, bundle ID, title, position, size, and active state; on-screen/minimized state is handled where the platform exposes it. CoreGraphics metadata has a top-level AX title/position/size fallback for background windows, without traversing their AX trees. The detection inventory is not copied into capture JSON or downstream processing. In `separate` and `primary` modes a safe-display capture JSON may still be written; when the active window is on the protected display, the gate suppresses its AX Tree, focused value, visible text, URL, and screenshot region. A foreground denylist match instead skips the entire capture. `all` skips the whole virtual-desktop image if any visible denied window is present. Set `off` to retain the old foreground-only behavior.
