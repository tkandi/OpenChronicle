...
- **`screenshot_privacy_mode`.** With `skip-monitor`, OpenChronicle inventories only alpha-positive, positive-size, normal layer-0 windows returned by CoreGraphics as on-screen immediately before each screenshot. App, bundle, and title rules apply to that inventory. If a CoreGraphics title is blank, top-level AX supplies only the title, and only after a globally unique exact match on the same PID and `CGWindowID`; AX position, size, or other geometry never authorizes fallback. A required missing, mismatched, duplicate, or ambiguous identity fails closed. Menus, popovers, and non-layer-0 floating panels are not independently protected as full-display windows by their titles; an app or bundle rule can still protect the capture when it independently matches an inventoried normal window or the foreground window. The helper does not traverse background AX trees. The detection inventory is not copied into capture JSON or downstream processing. In `separate` and `primary` modes a safe-display capture JSON may still be written; when the active window is on the protected display, the gate suppresses its AX Tree, focused value, visible text, URL, and screenshot region. A foreground denylist match instead skips the entire capture. `all` skips the whole virtual-desktop image if any denied inventoried window is present. Set `off` to retain the old foreground-only behavior.
