...
screenshot_privacy_fail_closed = true # abort the capture tick if window/display inventory fails
privacy_indicator_style = "pill"     # off | border | shield | pill | quiet-shield | banner
privacy_reason_display = "hybrid"    # overlay | diagnostics | hybrid
privacy_reason_detail = "exact"      # category | exact | tiered
privacy_reason_trigger = "hover"     # always | hover | click
...
protect_unknown_title_bundle_ids = [ # exact apps protected when title identity is unavailable
...
- **`screenshot_privacy_mode`.** The supported values are `off`, `skip-monitor`, `mask-window`, and `exclude-window`; the default remains `skip-monitor`. With `skip-monitor`, OpenChronicle inventories only alpha-positive, positive-size, normal layer-0 windows returned by CoreGraphics as on-screen immediately before capture. App, bundle, and title rules apply to that inventory. If a CoreGraphics title is blank, top-level AX supplies only the title, and only after a globally unique exact match on the same PID and `CGWindowID`; AX position, size, or other geometry never authorizes fallback. An unresolved title remains a typed unknown record instead of failing the complete inventory. When at least one title rule is configured, an unknown-title window is protected only if its exact Bundle ID is in `protect_unknown_title_bundle_ids`; reliable title matches remain global. If exact focused-window identity is unavailable, frontmost-PID layer-0 windows become active candidates and AX is suppressed only where a candidate display overlaps a protected display. Menus, popovers, and non-layer-0 floating panels are not independently protected as full-display windows by their titles. In `mask-window` and `exclude-window`, each matched protected/overlay window must resolve to one owning application and ScreenCaptureKit excludes that complete application, intentionally removing all of its normal and auxiliary windows. The helper compares complete pre/post shareable-content fingerprints before encoding PNG; a different application's privacy window that appears and disappears entirely between those snapshots remains outside what the OS APIs can prove absent. Genuine helper, parse, and display-inventory failures remain fixed-code failures. The helper does not traverse background AX trees, and the detection inventory is not copied into capture JSON or downstream processing. In `separate` and `primary` modes a safe-display capture JSON may still be written; `all` skips the whole virtual-desktop image if any denied inventoried window is present. Set `off` to disable the background guard and indicator while retaining every foreground app, bundle, title, URL, and text deny rule.
- **`screenshot_privacy_fail_closed`.** When true, a missing, timed-out, malformed, or invalid window/display inventory aborts the complete capture tick. When false, only legacy `skip-monitor` and `off` may capture without background protection after an ordinary inventory failure; stale indicators are cleared and visual confirmation remains false. `mask-window` and `exclude-window` always fail closed on inventory, ownership, helper, protocol, limit, image, or indicator-confirmation failure regardless of this setting. If the pause state cannot be read, OpenChronicle shows the yellow failed indicator and aborts the complete capture regardless of this setting. The default is true.
...
- **`privacy_reason_display`.** Chooses where protection reasons appear: `overlay`, the native Protection Diagnostics page, or `hybrid` for both. The default is `hybrid`. `diagnostics` sends no reason values to the overlay; the status indicator can remain visible, but `privacy_reason_trigger` has no overlay effect. `overlay` disables diagnostics-page subscription.
- **`privacy_reason_detail`.** `category` exposes only fixed reason codes. `exact` permits bounded app, bundle, title, rule, source-display, or resume-time fields only on an already protected overlay display or through a confirmed diagnostics lease. `tiered` keeps the overlay category-only and requires an explicit reveal in Protection Diagnostics. The default is `exact`.
- **`privacy_reason_trigger`.** Controls overlay detail reveal: `always`, pointer `hover`, or a small `click` target. The default is `hover`. The saved value is retained but has no effect when `privacy_indicator_style = "off"` or `privacy_reason_display = "diagnostics"`.
- **`protect_unknown_title_bundle_ids`.** Exact, case-insensitive Bundle IDs whose windows are protected when no trustworthy CoreGraphics or identity-authorized AX title can be read and at least one title pattern exists. The default is Edge (`com.microsoft.edgemac`), Chrome (`com.google.Chrome`), and Firefox (`org.mozilla.firefox`). An explicit empty list disables only this per-window uncertainty branch; reliable title matches, direct app/bundle matches, and complete inventory failures keep their existing behavior. This is a configurable risk scope, not browser private-mode detection.
- **Reason fallbacks.** The regular config loader normalizes invalid display/detail/trigger values to `hybrid`, `exact`, and `hover`; the native config editor rejects invalid values before writing. With indicator style `off`, overlay reasons are not rendered, while `diagnostics` and `hybrid` still use the owner-only diagnostics lease protocol.
- **macOS permissions.** The privacy helper requires Screen Recording permission, the same permission needed by screenshot capture. A missing individual title is represented conservatively per display; loss of the helper or its display inventory follows `screenshot_privacy_fail_closed`.
...
- **Denylist fields.** App/bundle lists are exact case-insensitive matches. The `*_patterns` lists are case-insensitive Python regular expressions. A foreground match skips the capture before JSON write, FTS indexing, timeline ingestion, and model processing. Window-title matches run before AX and screenshot capture; URL/text matches run after AX parsing but still before screenshot capture. The screenshot privacy guard can also apply app/bundle/title rules to non-focused windows in its normal layer-0 CoreGraphics inventory; URL/text rules remain foreground-only because it does not read background AX trees.
